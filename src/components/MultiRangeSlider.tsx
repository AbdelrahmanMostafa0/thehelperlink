...
import _styles from '@styles/multirangeslider.module.css';
...
const MultiRangeSlider = (props: Props, ref: React.ForwardedRef<HTMLDivElement>): JSX.Element => {
  ...
  return (
    <div ref={ref} className={styles.multiRangeSlider} style={props.style} onWheel={onMouseWheel}>
        ...
        <div className={styles.ruler}>
          {[...Array(stepCount)].map((e, i) => (
            <div key={i} className={styles.rulerRule}>
              {subSteps &&
                [...Array(10)].map((e, n) => <div key={n} className={styles.rulerSubRule}></div>)}
            </div>
          ))}
        </div>
      )}
